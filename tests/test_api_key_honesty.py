...
class TestZoteroAPIKeyHonesty:
    ...
    def test_zotero_source_validates_config(self, mock_get):
        ...
        with pytest.raises(
            ValueError, match="Invalid Zotero configuration.*API key is required"
        ):
            ZoteroSource(invalid_config)

        # Test auto-discovery failure with valid format but non-working key
        invalid_config = ZoteroConfig(
            api_key="abc123def456ghi789jkl012", library_type="user"
        )
    ...
    def test_cli_check_command_invalid_api_key_error(self):
        ...
        with patch("puby.zotero_source.zotero.Zotero") as mock_zotero_class:
            ...
            assert (
                "Invalid API key" in result.output
