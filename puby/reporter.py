...
class AnalysisReporter:
    ...
    def __init__(self, format: str = "table", verbose: bool = False):
    ...
    def print_sync_recommendations(self, recommendations: List[SyncRecommendation]):
    ...
    def _print_missing_section(self, missing_publications: List[Publication]):
    ...
    def _print_duplicates_section(self, duplicate_groups: List[List[Publication]]):
    ...
    def _print_potential_matches_section(self, potential_matches: List[PotentialMatch]):
        ...
        tuples = [(m.source_publication, m.reference_publication, m.confidence) 
                 for m in potential_matches]
        self.console_reporter.report_potential_matches(tuples)
    
    def _print_summary_statistics(self, result: AnalysisResult):
    ...
    def _select_primary_publication(self, publications: List[Publication]) -> Publication:
        ...
        def completeness_score(pub: Publication) -> int:
            ...
            score = 0
            if pub.title: score += 1
            if pub.authors: score += len(pub.authors)
            if pub.year: score += 1
            if pub.doi: score += 2  # DOI is more valuable
            if pub.journal: score += 1
            if pub.abstract: score += 1
            if pub.url: score += 1
            return score
